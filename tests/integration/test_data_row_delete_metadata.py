...
from labelbox.schema.data_row_metadata import DataRowMetadataField, DataRowMetadata, DataRowMetadataKind, DeleteDataRowMetadata, \
    DeleteDataRowMetadataV2, _parse_metadata_schema
...
def data_row_global_key(data_row):
    return GlobalKey(data_row.uid)


@pytest.mark.parametrize('data_row_for_delete',
                         ['data_row_unique_id', 'data_row_global_key'])
def test_bulk_delete_datarow_metadata_v2(data_row_for_delete, data_row, mdo,
                                         request):
    """test bulk deletes for all fields"""
    metadata = make_metadata(data_row.uid)
    mdo.bulk_upsert([metadata])
...
@pytest.mark.parametrize('data_row_for_delete',
                         ['data_row_unique_id', 'data_row_global_key'])
def test_bulk_partial_delete_datarow_metadata_v2(data_row_for_delete, data_row,
                                                 mdo, request):
    """Delete a single from metadata"""
    n_fields = len(mdo.bulk_export([data_row.uid])[0].fields)
    metadata = make_metadata(data_row.uid)
...
@pytest.mark.parametrize('data_rows_for_delete',
                         ['data_row_unique_ids', 'data_row_global_keys'])
def test_large_bulk_delete_datarow_metadata_v2(data_rows_for_delete,
                                               big_dataset, mdo, request):
    metadata = []
    data_row_ids = [dr.uid for dr in big_dataset.data_rows()]
    for data_row_id in data_row_ids:
        metadata.append(
...
@pytest.mark.parametrize('data_row_for_delete',
                         ['data_row_unique_id', 'data_row_global_key'])
def test_bulk_delete_datarow_enum_metadata_v2(data_row_for_delete,
...
def test_delete_non_existent_schema_id(data_row, mdo):
    res = mdo.bulk_delete([
        DeleteDataRowMetadataV2(data_row_id=data_row.uid,
                                fields=[SPLIT_SCHEMA_ID])
    ])
    assert len(res) == 1
...
@pytest.mark.parametrize('data_row_for_delete',
                         ['data_row_unique_id', 'data_row_global_key'])
def test_delete_non_existent_schema_id_v2(data_row_for_delete, data_row, mdo,
                                          request):
    res = mdo.bulk_delete([
        DeleteDataRowMetadataV2(
            data_row_id=request.getfixturevalue(data_row_for_delete),
